...
const isBookSourceMode = () => /bookSource/i.test(location.hash || location.href)
const createEmptySource = () =>
  JSON.parse(
    JSON.stringify(isBookSourceMode() ? emptyBookSource : emptyRssSource),
  ) as Source
...
export const useSourceStore = defineStore('source', {
  state: () => {
    return {
      ...
      currentSource: createEmptySource(), // 当前编辑的源
  ...
  getters: {
    sources: (state): Source[] =>
      isBookSourceMode() ? state.bookSources : state.rssSources,
  ...
  actions: {
    ...
    deleteSources(data: Source[]) {
      const sources: Source[] = isBookSourceMode()
        ? this.bookSources
        : this.rssSources
      data.forEach(source => {
    ...
    clearEdit() {
      ...
      this.currentSource = createEmptySource() //复制一份新对象
    },
